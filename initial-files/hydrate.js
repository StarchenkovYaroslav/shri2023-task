...
const TABS_ELEMENTS = TABS_KEYS.reduce((acc, key) => ({
  ...acc,
  [key]: TABS[key].items.map((item, index) =>
    <Event
      key={index}
      {...item}
    />)
...
function Tab() {
  ...
  React.useEffect(() => {
    ...
    const newHasRightScroll = totalWidth > ref.current.offsetWidth;
    ...
    <div className="section__panel-wrapper" ref={ref}>
      {TABS_KEYS.map(key =>
        <div key={key} role="tabpanel" className={'section__panel' + (key === activeTab ? '' : ' section__panel_hidden')} aria-hidden={key === activeTab ? 'false' : 'true'} id={`panel_${key}`} aria-labelledby={`tab_${key}`}>
          <ul className={'section__panel-list' + (key === activeTab ? ' section__panel-list_active' : '')}>
            {TABS_ELEMENTS[key]}
          </ul>
        </div>
      )}
...
function Main() {
  return <main className="main">
    ...
    <section className="section main__devices" id="tab-container">
      <Tab />
    </section>
  </main>;
}
